...
|---|---|---|---|
| `name` | JSON body | `required / string / max:100` | `Tokyo Taco Club` |
...
curl -i -X POST "$BASE_URL/api/v1/owner/carts" \
  ...
  --data '{"name":"Tokyo Taco Club","description":"Fresh tacos near Tokyo station.","cuisine":"Mexican","status":"open"}'
```

## Expected response and effects

owner_id comes from the token. moderation_status and is_featured cannot be set here. Minimum payload is name; status defaults to closed.
