# Delete Cart Schedule

`DELETE /api/v1/owner/carts/{cart}/schedules/{schedule}`
...
| `schedule` | Path | Existing resource ID | `501` |
...
```bash
curl -i -X DELETE "$BASE_URL/api/v1/owner/carts/401/schedules/501" \
...
Another owner gets 403. A nonexistent schedule or a schedule belonging to a different cart gets 404.
