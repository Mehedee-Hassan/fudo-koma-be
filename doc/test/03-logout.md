...
Revokes the current bearer token. Subsequent requests with that token must return 401. Remove the device registration first if signing out on a shared device.
