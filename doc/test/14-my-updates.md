# List Updates from Followed Carts
...
```bash
curl -i -X GET "$BASE_URL/api/v1/me/updates" --get \
...
Returns activity from the current user’s followed visible carts, with related cart data. Includes existing activity even when the follow was created later.
