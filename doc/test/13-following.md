...
```bash
curl -i -X GET "$BASE_URL/api/v1/me/following" --get \
...
Returns only active follows for the current user, with visible cart details. Hidden carts and disabled owners are excluded.
