...
`DELETE /api/v1/owner/carts/{cart}/photos/{photo}`
...
| `photo` | Path | Existing resource ID | `901` |
...
```bash
curl -i -X DELETE "$BASE_URL/api/v1/owner/carts/401/photos/901" \
...
Deletes the photo row and stored file. Another owner gets 403. Missing photo or photo on a different cart returns 404.
